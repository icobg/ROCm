...
if [ -d $ROCM_LIBRARIES_DIR ]; then
    ( cd $ROCM_LIBRARIES_DIR && git clean -f -d && git reset --hard HEAD && git fetch --all --tags && git checkout ${ROCM_BRANCH} )
else
    ( cd $ROCM_REL_DIR && git clone -j 4 --branch ${ROCM_BRANCH} https://github.com/ROCm/rocm-libraries.git )
fi

if [ -d $ROCM_SYSTEMS_DIR ]; then
    ( cd $ROCM_SYSTEMS_DIR && git clean -f -d && git reset --hard HEAD && git fetch --all --tags && git checkout ${ROCM_BRANCH} )
else
